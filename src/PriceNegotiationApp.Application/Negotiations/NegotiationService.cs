...
namespace PriceNegotiationApp.Application.Negotiations
{
    ...
    public class NegotiationService : INegotiationService
    {
        ...
        {
            var negotiation = await _context.Negotiations.FindAsync(new object[] { query }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Failed to find negotiation with ID '{query.Id}'"));
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Info Code Smell", "S1133:Deprecated code should be removed", Justification = "To be removed")]
        ...
        {
            var existingNegotiation = await _context.Negotiations.FindAsync(new object[] { command.Id }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Failed to find negotiation with ID '{command.Id}'"));
        ...
        {
            var negotiation = await _context.Negotiations.FindAsync(new object[] { id }, cancellationToken)
        ...
        {
            var existingNegotiation = await _context.Negotiations.FindAsync(new object[] { id }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Failed to find negotiation with ID '{id}'"));
        ...
        {
            var negotiation = await _context.Negotiations.FindAsync(new object[] { command.NegotiationId }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Failed to find negotiation with ID '{command.NegotiationId}'"));
        ...
        public async Task<NegotiationResultDto> CreateNegotiationAsync(CreateNegotiationCommand command, CancellationToken cancellationToken)
        {
            ...
            var product = await _context.Products.FindAsync(new object?[] { command.ProductId }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Product with ID '{command.ProductId}' was not found."));

            var customer = await _context.Customers.FirstOrDefaultAsync(x => x.IdentityId == userId, cancellationToken)
        ...
        {
            var negotiation = await _context.Negotiations.FindAsync(new object[] { id, cancellationToken }, cancellationToken)
                ?? throw new NotFoundException(string.Create(CultureInfo.InvariantCulture, $"Negotiation with ID '{id}' was not found."));
